...
export function detectAnomalies(): Anomaly[] {
  ...
  const anomalies: Anomaly[] = [
    ...
    },
    {
      ...
      severity: "critical",
      detectedAt: "2026-02-18T13:05:44+05:30",
      headline: `${C.CARD_TESTING.attempts} card attempts in 18 minutes, ${formatPercent(C.CARD_TESTING.declineRate, 0)} declined`,
      detail: `${C.CARD_TESTING.newProfiles} never-before-seen customer profiles ran ${C.CARD_TESTING.attempts} card authorizations averaging ₹${C.CARD_TESTING.avgAttemptedAmount} each in an 18-minute burst — a bot testing stolen card numbers, not real shoppers.`,
      impact: C.ANOMALY_LEDGER.card_testing.impact,
      recoverable: C.ANOMALY_LEDGER.card_testing.recoverable,
      confidence: 0.95,
      observedValue: C.CARD_TESTING.declineRate,
      baselineValue: C.CARD_TESTING.baselineDeclineRate,
      unit: "percent",
      affectedCount: C.CARD_TESTING.attempts,
      investigationId: "inv_1046",
      metricLabel: "Card decline rate",
    },
    {
      id: "anm_duplicate_charge",
      kind: "duplicate_charge",
      title: "Duplicate payment charges",
      severity: "critical",
      detectedAt: "2026-02-18T11:41:10+05:30",
      headline: `${C.DUPLICATE_CHARGE.pairs} customers billed twice for the same order`,
      detail: `A checkout retry after a gateway timeout re-submitted the payment instead of checking whether the first attempt had already succeeded, capturing ${C.DUPLICATE_CHARGE.pairs} customers twice between 11:10 and 11:40 IST.`,
      impact: C.ANOMALY_LEDGER.duplicate_charge.impact,
      recoverable: C.ANOMALY_LEDGER.duplicate_charge.recoverable,
      confidence: 0.97,
      observedValue: C.DUPLICATE_CHARGE.pairs,
      baselineValue: 0,
      unit: "count",
      affectedCount: C.DUPLICATE_CHARGE.pairs,
      investigationId: "inv_1047",
      metricLabel: "Duplicate captures",
    },
    {
